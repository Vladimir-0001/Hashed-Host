...
def redeem_licence(account,licence):
    now = int(time.time())
    expiry_time = now + int(licence[LICENCE_TIME])
    if account[ACCOUNT_EXPIRY_FIELD] < now:
        ...
        keys.delete_one({REDEEMABLE_KEY:licence[REDEEMABLE_KEY]})
    return expiry_time

def make_account(request,licence):
    ip_address = process_ip(get_ip(request))
    api_key = "hashed_" + secrets.token_hex(API_KEY_TOKEN_HEX_LENGTH)
    account = {
        API_KEY_FIELD:api_key,
        ACCOUNT_EXPIRY_FIELD: int(time.time())+int(licence[LICENCE_TIME]),
...
def verify_request(request, api_key, add = True, max_ips = ACCOUNT_MAX_IPS):
    ...
    if hashed_ip not in ip_list:
        if len(ip_list) < max_ips and add == True:
            ...
            return {'success': False, 'message':'IP not authorized please turn off any VPN or Proxy service. account sharing will lead to permanant termination of your account', 'code': 403}
...
def redeem():
    ...
    if api_key == None:
        licence = keys.find_one({REDEEMABLE_KEY: key})
        if not licence:
            return {'error':'licence Key not Found '}, 400
        return make_account(request,licence)
        ELE
    else:
        print(api_key)
        account = check_api_key(api_key)
        licence = keys.find_one({REDEEMABLE_KEY: key})
        if not licence:
            return{'error':'invalid licence key'}, 400 
        if not account:
            return {'error':'Api key Invalid '}, 400
        expiry  = redeem_licence(account,licence)
